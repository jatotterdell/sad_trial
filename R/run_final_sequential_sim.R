...
epsilon <- list(c(0.975, 0.05, 0.025, 0.85))
Delta <- list(c(2.5, 5, 5))
stop_rule <- list(1)

scenarios <- scenario_list(n_list(p, mu0, Sigma0, epsilon, Delta, stop_rule, looks))
...
set.seed(652647)
res_trial <- rbindlist(pbapply::pblapply(scenarios, function(x) sim_replicate(1e4, trial_par = x, cores = 15)), idcol = "scenario")
...
saveRDS(scenarios_dt, file = "~/out_files/mental_health_sims/basic_sequential_scenarios_final.rds")
saveRDS(dat_trial, file = "~/out_files/mental_health_sims/basic_sequential_sims_final.rds")
saveRDS(dat_sum, file = "~/out_files/mental_health_sims/basic_sequential_summary_final.rds")
